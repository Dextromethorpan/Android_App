...
class MainActivity2 : AppCompatActivity() {
    ...
    private lateinit var chordTimeline   : LinearLayout
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        chordEngine2   = ChordEngine(this)

        tvStatus.text = "Loading…"
        lifecycleScope.launch(Dispatchers.IO) {
            runCatching { PianoSynth.init() }
            runCatching { SoundFontPlayer.init(this@MainActivity2) }
            runCatching { chordEngine2?.load() }
            runCatching { chordSeqRunner.load() }
            withContext(Dispatchers.Main) { tvStatus.text = "Ready ✓" }
        }
    ...
    private fun buildTopBar(): View {
        ...
        tvStatus = TextView(this).apply { text = "Starting…"; textSize = 9f; setTextColor(C.TXT_HINT) }
    ...
    private fun buildKeySelector(): View {
        ...
        val piano = PianoKeySelectorView(this) { key ->
            selectedKey = key; tvSelectedKey.text = key
    ...
    private fun buildStylePickers(): View {
        ...
        val genres  = listOf("Jazz","Blues","Rock","Pop","Soul / R&B","Folk / Country","Funk","Bossa Nova","Electronic")
    ...
    private fun buildRecordSection(): View {
        ...
        btnGenerate = TextView(this).apply {
            ...
            setTextColor(C.PURPLE_LITE); setBackgroundColor(C.PURPLE_DARK)
        ...
        tvDetectedNotes = TextView(this).apply {
            text = "Detected notes appear here…"; textSize = 11f; setTextColor(C.TXT_MUTED)
            typeface = Typeface.MONOSPACE; setPadding(dp(14), dp(4), dp(14), dp(8))
        }
        ...
        tvKeyWarning = TextView(this).apply {
            ...
            setPadding(dp(14), 0, dp(14), dp(8)); visibility = View.GONE
        }
    ...
    private fun buildChordSection(): View {
        ...
        for (i in 0..3) {
            ...
            val slot = LinearLayout(this).apply {
                ...
                setBackgroundColor(C.BG_SECTION); alpha = 0.35f
            }
            slot.addView(TextView(this).apply {
                text = "—"; textSize = 13f
                setTypeface(null, Typeface.BOLD); setTextColor(C.TXT_HINT); gravity = Gravity.CENTER
            })
    ...
    private fun buildTracksSection(): View {
        ...
        playAllRow.addView(secLabel("⑤ instrument tracks").apply { setPadding(0,0,0,0) }, lp(0, WRAP) { weight = 1f })
        playAllRow.addView(TextView(this).apply {
            text = "▶ Play All"; textSize = 11f; gravity = Gravity.CENTER
            setTypeface(null, Typeface.BOLD); setTextColor(C.PURPLE_LITE); setBackgroundColor(C.PURPLE)
        ...
        val colors = listOf(
            C.SLOTS[0], C.SLOTS[1], C.SLOTS[2], C.SLOTS[2],
        ...
        instruments.forEachIndexed { i, (name, rule) ->
            ...
            val header = LinearLayout(this).apply {
                orientation = LinearLayout.HORIZONTAL
                setPadding(dp(14), dp(8), dp(14), dp(4)); gravity = Gravity.CENTER_VERTICAL
            }
            header.addView(TextView(this).apply {
                text = name; textSize = 11f; setTypeface(null, Typeface.BOLD); setTextColor(C.TXT_HINT)
            }, lp(0, WRAP) { weight = 1f })
            header.addView(TextView(this).apply { text = rule; textSize = 9f; setTextColor(C.TXT_HINT) })
            header.addView(TextView(this).apply {
            ...
            for (s in 0..3) {
                ...
                val slot = LinearLayout(this).apply {
                    orientation = LinearLayout.VERTICAL
                    setPadding(dp(8), dp(7), dp(8), 0); setBackgroundColor(sc[0])
                }
                val valueText = TextView(this).apply {
                    text = "—"; textSize = 12f; setTypeface(null, Typeface.BOLD); setTextColor(sc[1])
                }
                slot.addView(valueText)
                slot.addView(TextView(this).apply {
                    text = listOf("I","II","III","IV")[s]; textSize = 9f; setTextColor(sc[2])
                })
                slot.addView(View(this).apply {
            ...
            wrapper.addView(roll, LinearLayout.LayoutParams(MATCH, rollH).apply { setMargins(dp(14), dp(4), dp(14), 0) })
    ...
    // ── Track playback using SoundFontPlayer ──────────────────────────────────
    ...
    private fun onPlayTrackClicked(trackIdx: Int) {
        ...
        val notesList: List<String>? = when (trackIdx) {
            ...
            2 -> tracks.piano.map { p -> p.substringAfter("LH:").substringBefore(" ").split("+").firstOrNull() ?: "C3" }
            3 -> tracks.piano.map { p -> p.substringAfter("RH:").split("+").firstOrNull() ?: "E4" }
            4 -> tracks.pads
            5 -> tracks.leadMelody
        ...
        if (notesList != null) {
            lifecycleScope.launch {
                notesList.forEachIndexed { ci, noteName ->
                    ...
                    SoundFontPlayer.playTrack(trackIdx, listOf(midi), durationMs = 1400)
                    kotlinx.coroutines.delay(1700)
                }
                trackRolls.getOrNull(trackIdx)?.highlightChord(-1)
            }
        } else {
            // Percussion — play using SF2 file via SoundFontPlayer
            val drumMidi = when (trackIdx) {
                7  -> listOf(36)   // Kick — low note via Mystic Bass Synth
                8  -> listOf(60)   // Snare — mid note via Sawtooth Pad
                9  -> listOf(72)   // Hi-Hat — high note via Theremin
                10 -> listOf(65)   // Crash — mid-high via Oohs
                else -> return
            }
            lifecycleScope.launch {
                ...
                repeat(chordCount) { ci ->
                    trackRolls.getOrNull(trackIdx)?.highlightChord(ci)
                    val hitsPerChord = when (trackIdx) { 9 -> 4; else -> 2 }
                    repeat(hitsPerChord) {
                        SoundFontPlayer.playTrack(trackIdx, drumMidi, durationMs = 200)
    ...
    private fun onPlayAllClicked() {
        ...
        playAllJob = lifecycleScope.launch {
            ...
            for (ci in 0 until chordCount) {
                ...
                val lhNote = tracks.piano.getOrNull(ci)
                    ?.substringAfter("LH:")?.substringBefore("+")?.trim()

                // Play each track with its own SF2 simultaneously
                val trackNoteMap = mapOf(
                    0 to listOfNotNull(tracks.bass.getOrNull(ci)?.let { MidiExporter.noteNameToMidi(it) }),
                    2 to listOfNotNull(lhNote?.let { MidiExporter.noteNameToMidi(it) }),
                    4 to listOfNotNull(tracks.pads.getOrNull(ci)?.let { MidiExporter.noteNameToMidi(it) }),
                    5 to listOfNotNull(tracks.leadMelody.getOrNull(ci)?.let { MidiExporter.noteNameToMidi(it) }),
                    6 to listOfNotNull(tracks.counterMelody.getOrNull(ci)?.let { MidiExporter.noteNameToMidi(it) })
                )
                trackNoteMap.forEach { (trackIdx, notes) ->
                    if (notes.isNotEmpty()) SoundFontPlayer.playTrack(trackIdx, notes, durationMs = 1400)
                }
                // Kick on every chord
                SoundFontPlayer.playTrack(7, listOf(36), durationMs = 200)
            ...
            runCatching {
                pitchDetector.startRecording { note ->
                    runOnUiThread {
                        ...
                        tvDetectedNotes.text = if (current == "🎤 Listening…") note else "$current  $note"
                    }
                }
            }.onFailure { e ->
    ...
    private fun displayTracks(tracks: ChordDeriver.TrackResult) {
        ...
        val allValues = listOf(
            tracks.bass, tracks.rhythmGuitar, lhNotes, rhNotes, tracks.pads,
        ...
        tvList.forEachIndexed { i, tv ->
            ...
            slots?.forEachIndexed { s, slotTv ->
                slotTv.text = values.getOrElse(s) { "—" }; slotTv.alpha = 1f
            }
        }
    ...
    private fun buildExportSection(): View {
        ...
        listOf(1, 2, 4).forEach { bars ->
            val btn = TextView(this).apply {
                ...
                setPadding(dp(14), dp(7), dp(14), dp(7)); tag = bars
            }
    ...
    private fun showExportSection() { exportSection.visibility = View.VISIBLE }
    ...
    private fun onExportClicked() {
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            runCatching {
                val midi = MidiExporter.export(tracks = tracks, chords = currentChords, bpm = 120, barsPerChord = barsPerChord)
                val fileName = "ChordsPro_${System.currentTimeMillis()}.mid"
                val downloadsDir = android.os.Environment.getExternalStoragePublicDirectory(android.os.Environment.DIRECTORY_DOWNLOADS)
                val file = java.io.File(downloadsDir, fileName)
                file.writeBytes(midi)
                val uri = androidx.core.content.FileProvider.getUriForFile(this@MainActivity2, "${packageName}.provider", file)
    ...
    private fun spinnerCard(label: String, items: List<String>, onSelected: (String) -> Unit): View {
        val col = LinearLayout(this).apply {
            orientation = LinearLayout.VERTICAL; setBackgroundColor(C.BG_SECTION)
            setPadding(dp(10), dp(8), dp(10), dp(8))
        }
        col.addView(TextView(this).apply { text = label; textSize = 9f; setTextColor(C.TXT_HINT) })
        val valueText = TextView(this).apply {
            text = items.first(); textSize = 13f; setTextColor(C.TXT_PRIMARY); setTypeface(null, Typeface.BOLD)
        }
        col.addView(valueText)
        col.setOnClickListener {
            AlertDialog.Builder(this).setTitle(label)
                .setItems(items.toTypedArray()) { _, pos -> valueText.text = items[pos]; onSelected(items[pos]) }
                .show()
        }
        return col
    }
...
class TrackMiniRollView(
    ...
    private val midiToRow  = mapOf(60 to 0, 59 to 2, 58 to 3, 57 to 4, 56 to 5, 55 to 6, 53 to 7, 52 to 9, 51 to 10, 50 to 11, 49 to 12, 48 to 13)
    private val KEY_W = 44f; private val ROWS = 14
    ...
    private val txtPaint  = android.graphics.Paint(android.graphics.Paint.ANTI_ALIAS_FLAG).apply { color = android.graphics.Color.parseColor("#333352"); textAlign = android.graphics.Paint.Align.LEFT }
    private val lblPaint  = android.graphics.Paint(android.graphics.Paint.ANTI_ALIAS_FLAG).apply { textAlign = android.graphics.Paint.Align.LEFT }
    private val phPaint   = android.graphics.Paint(android.graphics.Paint.ANTI_ALIAS_FLAG).apply { color = C.ORANGE }
    fun setNotes(names: List<String>, eng: ChordEngine?) { noteNames = names; engine = eng; invalidate() }
    ...
    override fun onDraw(canvas: android.graphics.Canvas) {
        ...
        keyLabels.forEachIndexed { i, _ ->
            bgPaint.color = if (keyIsBlack[i]) android.graphics.Color.parseColor("#0D0D18") else android.graphics.Color.parseColor("#111120")
            canvas.drawRect(0f, i * rowH, w, (i+1) * rowH, bgPaint)
        }
        linePaint.color = android.graphics.Color.parseColor("#363650"); linePaint.strokeWidth = 1.5f
        ...
        if (noteNames.isNotEmpty()) {
            ...
            for (ci in 0 until noteNames.size) {
                if (ci > 0) { linePaint.color = android.graphics.Color.parseColor("#363650"); linePaint.strokeWidth = 1.5f; canvas.drawLine(KEY_W + ci * slotW, 0f, KEY_W + ci * slotW, h, linePaint) }
                linePaint.color = android.graphics.Color.parseColor("#1E1E2E"); linePaint.strokeWidth = 0.5f
                for (b in 1..3) canvas.drawLine(KEY_W + ci * slotW + slotW * b / 4f, 0f, KEY_W + ci * slotW + slotW * b / 4f, h, linePaint)
            }
        }
        linePaint.color = android.graphics.Color.parseColor("#1A1A2A"); linePaint.strokeWidth = 0.5f
        keyLabels.indices.forEach { i -> canvas.drawLine(0f, i * rowH, KEY_W, i * rowH, linePaint) }
        keyLabels.forEachIndexed { i, label -> if (label.isNotEmpty()) canvas.drawText(label, 3f, i * rowH + rowH * 0.72f, txtPaint) }
        if (noteNames.isNotEmpty()) {
            val slotW = gridW / noteNames.size
            lblPaint.textSize = (fontSize * 0.85f).coerceIn(9f, 16f); lblPaint.color = accentColor
            noteNames.forEachIndexed { ci, name -> canvas.drawText(name, KEY_W + ci * slotW + 4f, rowH * 0.85f, lblPaint) }
        }
        if (noteNames.isEmpty()) {
            notePaint.color = android.graphics.Color.parseColor("#222238"); notePaint.alpha = 80
            ...
            notePaint.alpha = 255; return
        }
        val slotW = gridW / noteNames.size
        noteNames.forEachIndexed { ci, noteName ->
            val isActive = ci == activeChord; val nx = KEY_W + ci * slotW + 3f; val nw = slotW * 0.84f
            val midiList: List<Int> = engine?.notesForChord(noteName)?.filter { it in 48..60 }?.takeIf { it.isNotEmpty() }
                ?: MidiExporter.noteNameToMidi(noteName.trim())?.takeIf { it in 48..60 }?.let { listOf(it) } ?: listOf(53)
            val displayRows = midiList.mapNotNull { midiToRow[it] }.ifEmpty { listOf(6) }
            displayRows.forEachIndexed { ni, row ->
                notePaint.color = accentColor
                notePaint.alpha = when { isActive && ni == 0 -> 255; isActive -> 160; ni == 0 -> 220; else -> 130 }
                val y = row * rowH + 1f
                canvas.drawRoundRect(nx, y, nx + nw, y + rowH - 2f, 3f, 3f, notePaint)
                if (ni == 0) { notePaint.color = android.graphics.Color.WHITE; notePaint.alpha = 40; canvas.drawRoundRect(nx, y, nx + nw, y + rowH * 0.3f, 3f, 3f, notePaint) }
            }
        }
        notePaint.alpha = 255
        if (activeChord >= 0 && noteNames.isNotEmpty()) {
            val slotW2 = gridW / noteNames.size; val px = KEY_W + activeChord * slotW2 + slotW2 * 0.5f
            phPaint.style = android.graphics.Paint.Style.STROKE; phPaint.strokeWidth = 2.5f
            canvas.drawLine(px, 0f, px, h, phPaint)
            phPaint.style = android.graphics.Paint.Style.FILL
            canvas.drawPath(android.graphics.Path().apply { moveTo(px - 6f, 0f); lineTo(px + 6f, 0f); lineTo(px, 10f); close() }, phPaint)
        }
    }
}